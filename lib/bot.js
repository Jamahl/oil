...
const STATE_PATH = path.join(__dirname, '..', 'data', 'bot_state.json');
...
// Hot momentum + runner enabled -> split into banker (fixed TP) + runner
  ...
  for (const tk of tickets) {
    try {
      ...
      state.open.push({ dealId, dir, size: tk.size, entry: conf.level || entryRef, sl: stopLevel, tp: tk.trailing ? null : profitLevel, kind: tk.kind, at: new Date().toISOString() });
...
module.exports = { tick, start, stop, closeAll, manual, setConfig, status };
 + sl.toFixed(2) : 'SL ' + stopLevel.toFixed(2) + ' TP ' + profitLevel.toFixed(2)}${hot ? ' [hot momentum ' + momo.score + ']' : ''}`);
    } catch (e) {
      log(tk.kind + ' entry failed: ' + e.message);
    }
  }

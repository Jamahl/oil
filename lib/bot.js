...
try {
  Object.assign(state, JSON.parse((() => { try { return fs.readFileSync(STATE_PATH, 'utf8'); } catch (e) { return fs.readFileSync(STATE_PATH + '.bak', 'utf8'); } })()));
...
async function tick(signal, spot) {
  ...
  const size = c.sizeMode === 'fixed' ? c.positionSize : Math.max(1, Math.floor((c.riskAmount / sl) * 10) / 10);
...
function switchEnv(env) {
  ...
  try { const saved = JSON.parse((() => { try { return fs.readFileSync(STATE_PATH, 'utf8'); } catch (e) { return fs.readFileSync(STATE_PATH + '.bak', 'utf8'); } })()); Object.assign(state, saved); state.running = false; state.config = { ...DEFAULT_CONFIG, ...(env === 'live' ? LIVE_SAFE : {}), ...saved.config }; } catch {}

...
const DEFAULT_CONFIG = {
  ...
  maxHoldMin: 0, // force-close a scalp that hits neither TP nor SL in N min (0 = off)
  ...
  async function dbInsertOpen(t) {
    try {
      ...
      await d.run(
        ...
        [inst.id, envName, t.dealId, t.kind || null, t.dir, numOrNull(t.size), numOrNull(t.entry), numOrNull(t.sl), numOrNull(t.tp), tsMs(t.at) || Date.now()]
      );
    ...
    try {
      ...
      for (const t of [...state.open]) {
        if (!brokerIds.has(t.dealId)) {
          ...
          await dbMarkClosed(rec); // mirror the close into the shared ledger
  ...
  async function tick(signal, spot) {
    ...
    for (const tk of tickets) {
      try {
        ...
        const trade = { dealId, dir, size: tk.size, entry: conf.level || entryRef, sl: stopLevel, tp: tk.trailing ? null : profitLevel, kind: tk.kind, env: envName, at: new Date().toISOString() };

...
async function tick(signal, spot) {
  ...
  try {
    const res = await capital.openPosition({
      ...
      profitLevel: Math.round(profitLevel * 100) / 100,
    });
    ...
    log(`OPEN ${dir} ${size} @${(conf.level || entryRef).toFixed(2)} SL ${stopLevel.toFixed(2)} TP ${profitLevel.toFixed(2)} (${signal.confidence}, bias ${signal.bias})`);
  } catch (e) {
    log('entry failed: ' + e.message);
  }
  save();
}

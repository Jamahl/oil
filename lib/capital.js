...
const env = () => (creds() || {}).env;
const openPosition = ({ direction, size, stopLevel, profitLevel, epic = EPICS.brent }) =>
  dealReq('POST', '/positions', { epic, direction, size, guaranteedStop: false, stopLevel, profitLevel });
...
module.exports = { snapshot, configured: () => Boolean(creds()), env, openPosition, closePosition, listPositions, confirmDeal, accounts };

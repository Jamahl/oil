...
// Any EIA weekly series from the public dnav history workbook (no API key):
// https://www.eia.gov/dnav/pet/hist_xls/{ID}w.xls -> { weekEnd[], value[] } ascending.
async function eiaWeeklySeries(seriesId) {
  const key = `eia_${seriesId.toLowerCase()}_v2`; // v2: {value} shape (v1 cached {kbbl})
  const hit = readCache(key);
  if (hit) return hit;
  const res = await fetchWithRetry(`https://www.eia.gov/dnav/pet/hist_xls/${seriesId}w.xls`);
  const buf = Buffer.from(await res.arrayBuffer());
  const wb = XLSX.read(buf, { type: 'buffer' });
  const ws = wb.Sheets['Data 1'];
  if (!ws) throw new Error(`EIA: sheet "Data 1" missing in ${seriesId}w.xls`);
  ...
  if (!weekEnd.length) throw new Error(`EIA: no data rows parsed from ${seriesId}w.xls`);
  const out = { weekEnd, value };
  writeCache(key, out);
  return out;
}

// U.S. ending stocks of crude oil excl SPR (thousand barrels) — legacy shape.
async function eiaCrudeStocks() {
  const s = await eiaWeeklySeries('WCESTUS1');
  return { weekEnd: s.weekEnd, kbbl: s.value };
}

// CFTC legacy COT (futures-only) for NYMEX WTI (code 067651) via the public
// Socrata API — non-commercial net positioning as the oil-complex crowdedness
// proxy. As-of date is Tuesday; published Friday ~15:30 ET.
// -> { asOf[], net[], oi[] } ascending.
async function cotWtiPositioning() {
  const key = 'cot_wti_legacy';
  const hit = readCache(key, 24 * 60 * 60 * 1000);
  if (hit) return hit;
  const url =
  ...
  for (const r of json) {
    ...
    const o = parseFloat(r.open_interest_all);
    if (!isFinite(long) || !isFinite(short) || !isFinite(o) || o <= 0) continue;
    asOf.push(r.report_date_as_yyyy_mm_dd.slice(0, 10));
    net.push(long - short);
    oi.push(o);
  }
  if (!asOf.length) throw new Error('CFTC: no COT rows parsed');
  const out = { asOf, net, oi };
  writeCache(key, out);
  return out;
}

module.exports = { yahooDaily, yahooSeries, eiaCrudeStocks, eiaWeeklySeries, cotWtiPositioning, clearCache, readCache, writeCache, fetchWithRetry };

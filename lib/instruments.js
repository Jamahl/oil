...
const INSTRUMENTS = {
  ...
  btc: {
    ...
    botDefaults: {
      ...
      slMode: 'pct',
      slValue: 0.7,
      // Anti-jam rails (2026-07-18): fixed-pct targets are unhittable on a flat
      // day, so stale scalps must expire, and an opposite signal should close
      // the other side instead of stacking a hedged book at max slots.
      maxHoldMin: 15,
      closeOnFlip: true,
      // BTC is ~1.5× more volatile than Brent; target vol reflects that
      volTarget: 1.5,
      // BTC trades 24/7 — let all hours through until we have BTC-specific hour analysis
      killHours: [],
    },
  ...
  gold: {
    ...
    newsPack: {
      cacheKey: 'news_raw_gold',
      parallelCacheKey: 'news_parallel_gold',
      llmCachePrefix: 'news_llm_gold',
      promptVersion: 'v1',
      tier1: ['fomc', 'rate cut', 'rate hike', 'cpi', 'nonfarm', 'payroll', 'inflation', 'recession', 'dollar', 'usd'],
      tier2: ['fed', 'treasury', 'yield', 'bond', 'dxy', 'haven', 'safe haven', 'geopolitical', 'central bank', 'reserve', 'xau'],
      topicRe: /(gold|xau|precious|bullion|fomc|fed|inflation|dollar|haven|rates|yield)/i,
      feeds: [
        { url: 'https://news.google.com/rss/search?q=gold+OR+XAU+OR+precious+metals+when:2d&hl=en-US&gl=US&ceid=US:en', label: 'Google News' },
        { url: 'https://news.google.com/rss/search?q=(gold+OR+XAU+OR+precious+metals)+source:bloomberg+when:2d&hl=en-US&gl=US&ceid=US:en', label: 'Bloomberg' },
        { url: 'https://news.google.com/rss/search?q=(gold+OR+XAU+OR+precious+metals)+source:reuters+when:2d&hl=en-US&gl=US&ceid=US:en', label: 'Reuters' },
        { url: 'https://www.kitco.com/rss/', label: 'Kitco' },
      ],
      parallelObjective: 'Breaking news from the past 24-48 hours moving the GOLD price: Fed/FOMC decisions, rate expectations, CPI/inflation data, dollar strength, geopolitical tensions, central bank gold reserves, safe-haven flows. Only recent news reports — no explainers or evergreen pages.',
      parallelQueries: ['gold price news today', 'gold XAU FOMC Fed rate precious metals latest'],
      systemPrompt: `You are a senior precious-metals market analyst writing for a discretionary Gold trader. You will get a numbered list of recent headlines with source and age. Judge each headline's likely effect on the GOLD price over the next days — not whether the news is good or bad for the world.

Reply ONLY minified JSON, no prose, no code fences:
{"items":[{"i":<index>,"direction":"bull"|"bear"|"unclear","materiality":1|2|3,"novelty":"new"|"update"|"rehash"}],"overall":{"lean":"bullish"|"bearish"|"mixed"|"quiet","summary":"<=140 chars"}}

DIRECTION — think macro and flows for gold:
- bull: dovish Fed (rate cuts, weaker dollar, lower real yields), hot inflation (CPI/PCE above expectations), geopolitical turmoil/uncertainty, safe-haven buying, central bank reserve accumulation, falling equities, weak nonfarm payrolls, recession fears, falling bond yields, stagflation signals.
- bear: hawkish Fed (rate hikes, higher-for-longer, strong dollar), strong economic data (hot NFP, strong GDP, tame inflation), rising real yields, risk-on rotation (equities rallying), dollar strength, gold ETF outflows, ceasefire/detente reducing safe-haven demand, Treasury rally reducing gold's appeal.
- Traps to get right: A "dollar stronger" headline is BEAR for gold. A "rate cut" headline is BULL for gold. A "stocks rally" headline is typically BEAR for gold (risk-on rotation). "Inflation above expectations" is BULL for gold (inflation hedge narrative). "Geopolitical tensions ease" is BEAR (safe-haven premium evaporates). If genuinely ambiguous, say "unclear" — do not guess.

MATERIALITY — would this move gold today?
- 3: could move gold >1% (FOMC rate decision surprise, major CPI/NFP miss, unexpected Fed pivot, major geopolitical escalation, central bank reserve announcement from a large economy).
- 2: notable but incremental (Fed-speak shift, data prints within expectations, moderate geopolitical developments, gold ETF flow prints).
- 1: background — opinion, explainer, mining stock coverage, price recaps, small updates.

NOVELTY:
- new: first report of a fresh event.
- update: meaningful new development in a known story.
- rehash: re-reporting or commentary on something days old.

OVERALL — lean = net NEAR-TERM price pressure from THIS set of headlines. "quiet" if nothing material. summary: name the single dominant driver, plain language, <=140 chars.`,
      scoreLabel: 'gold/precious-metals headlines',
      sourcesLine: 'newest first · updates every 5 min · Google News / Bloomberg / Reuters / Kitco + Parallel',
    },
  },
};

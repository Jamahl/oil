...
async function logSignal(sig) {
  ...
  const row = await d.get('SELECT at FROM signals WHERE at = ?', [at]);
  if (row) return;
  await d.run('INSERT INTO signals (at, signal, bias, confidence, tape, price) VALUES (?, ?, ?, ?, ?, ?)', [
    at, sig.signal, sig.bias, sig.confidence || null, sig.tape || null, sig.price,
  ]);
}

async function resolveSignals(fallbackSeries) {
  const d = await getDriver();
  const now = Date.now();
  const jobs = [
  ...
  for (const j of jobs) {
    const rows = await d.all(`SELECT at, signal, price FROM signals WHERE ${j.col} IS NULL AND at <= ?`, [now - j.ms]);
    for (const r of rows) {
      const found = await lookupRealized(d, Number(r.at) + j.ms, j.windowMs, fallbackSeries);
      if (!found) {
        if (now > Number(r.at) + j.ms + j.windowMs) {
          await d.run(`UPDATE signals SET ${j.col} = 0, ${j.hit} = NULL WHERE at = ?`, [r.at]); // unresolvable — parked
        }
        continue;
      }
      const ret = found.price / r.price - 1;
      const hit = r.signal === 'HOLD' || ret === 0 ? null : (r.signal === 'BUY') === ret > 0 ? 1 : 0;
      await d.run(`UPDATE signals SET ${j.col} = ?, ${j.hit} = ? WHERE at = ?`, [ret, hit, r.at]);
    }
  }
}

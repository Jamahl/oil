...
function ddl(idLine) {
  return `
    ...
    -- Scalp-bot trade ledger. Every entry INSERTs a status='open' row; every
    -- close UPDATEs it to 'closed' (matched by deal_id). lib/bot.js dual-writes
    -- here and to its JSON state file, and reads history/stats/open-set back from
    -- here so trade history travels between machines (any host with DATABASE_URL
    -- sees the same record); the JSON file is the automatic fallback on a DB
    -- outage. UNIQUE (instrument, env, deal_id) makes writes idempotent — a
    -- re-inserted deal is a no-op, and the one-time JSON import dedupes on it.
    CREATE TABLE IF NOT EXISTS bot_trades (
      ...
      UNIQUE (instrument, env, deal_id)
    );
    CREATE INDEX IF NOT EXISTS idx_bot_trades_lookup ON bot_trades(instrument, env, status);
  `;
}
...
async function getDriver() {
  ...
  driverPromise = (async () => {
    if (process.env.DATABASE_URL) {
      ...
      // Idempotent migration: add the signal_snapshot column to pre-existing
      // bot_trades tables (born without it). Nullable so old rows and any
      // failed-write path keep working. Stores a JSON string; the dual-driver
      // SQL surface is TEXT (sqlite has no JSONB), Postgres accepts TEXT here.
      await pool.query(`ALTER TABLE bot_trades ADD COLUMN IF NOT EXISTS signal_snapshot TEXT`);
    ...
    // Idempotent migration: add signal_snapshot to bot_trades if missing
    // (sqlite has no ADD COLUMN IF NOT EXISTS, so check the column list).
    const hasSignalSnapshot = db.prepare('PRAGMA table_info(bot_trades)').all().some((c) => c.name === 'signal_snapshot');
    if (!hasSignalSnapshot) db.exec('ALTER TABLE bot_trades ADD COLUMN signal_snapshot TEXT');
...
module.exports = { getDriver, logPrice, logPredictions, resolveDue, computeCalibration, stats, storageKind, logSignal, resolveSignals, signalStats, HORIZONS };

...
async function scoreNews(items, model, apiKey) {
  ...
  const cacheKey = `news_llm_${PROMPT_VERSION}_${slug.replace(/[^a-z0-9.-]+/gi, '_')}_${djb2(items.map((i) => i.title).join('|'))}`;
    ...
    'https://openrouter.ai/api/v1/chat/completions',
    {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Score these crude-oil headlines:\n${list}` },
        ],
      }),
    },
    2

...
const HORIZON_BARS = { fwd1: 1, fwd5: 5, fwd21: 21 };
...
// Round-trip trading cost for the sign strategy, per side (Brent CFD/futures
// spread ~4c on $80 ≈ 5bps round trip; 3bps/side is deliberately conservative).
const DEFAULT_COST_PER_SIDE = 0.0003;

// Metrics on the out-of-sample segment. stride > 1 evaluates non-overlapping
// windows (the honest number for multi-day horizons). opts.from/opts.to restrict
// to a date window (research holdout scoring); opts.costPerSide overrides the
// default trading cost.
function evaluate(rows, preds, horizonKey, stride = 1, opts = {}) {
  const costPerSide = opts.costPerSide != null ? opts.costPerSide : DEFAULT_COST_PER_SIDE;
  const pts = [];
  for (let i = 0; i < rows.length; i++) {
    if (preds[i] == null || rows[i][horizonKey] == null) continue;
    if (opts.from && rows[i].date < opts.from) continue;
    if (opts.to && rows[i].date >= opts.to) continue;
    pts.push({ pred: preds[i], actual: rows[i][horizonKey], date: rows[i].date });
  }
  ...
  let nTrades = 0;
  for (const p of pts) {
    const pos = Math.sign(p.pred);
    const r = (pos * p.actual) / h;
    if (pos !== prevPos) nTrades++;
    netRets.push(r - Math.abs(pos - prevPos) * costPerSide);
    prevPos = pos;
    stratRets.push(r);
    e *= 1 + r;
    eb *= 1 + p.actual / h;
  ...
  const srNet = std(netRets) > 0 ? (mean(netRets) / std(netRets)) * Math.sqrt(252) : 0;
...
function computeBundle({ rows, features }, kind, horizonKey, opts = {}) {
  ...
  const HORIZON_LABELS = { fwd1: '1d', fwd5: '5d', fwd21: '21d' };
  return {
    kind,

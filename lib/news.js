...
// Tier lists follow the CrudeSignal PRD activity score (oil) with a parallel
// set for gold (Fed/rates, safe-haven, central-bank flows).
const OIL_TIER1 = [
...
const GOLD_TIER1 = [
  'fomc', 'rate cut', 'rate hike', 'fed pivot', 'cpi', 'nonfarm payrolls',
  'central bank buying', 'central bank gold', 'war escalat', 'escalation',
  'invasion', 'attack', 'sanction', 'emergency meeting', 'bank collapse', 'default',
];
const GOLD_TIER2 = [
  'inflation', 'dollar', 'treasury yield', 'real yield', 'safe haven', 'etf',
  'bullion', 'recession', 'tariff', 'trade war', 'geopolit', 'china', 'india',
  'jewellery', 'jewelry', 'mine', 'comex', 'fed', 'powell', 'stimulus',
];

function scoreText(text, tier1, tier2) {
...
const GOLD_TOPIC = /(gold|bullion|xau|precious metal|silver|platinum)/i;
...
// `topicFilter` feeds are general-news firehoses kept only where the title is on-topic.
const OIL_FEEDS = [
...
const GOLD_FEEDS = [
  { url: 'https://news.google.com/rss/search?q=gold+price+OR+bullion+OR+%22gold+futures%22+when:2d&hl=en-US&gl=US&ceid=US:en', label: 'Google News' },
  { url: 'https://news.google.com/rss/search?q=(gold+OR+bullion)+source:bloomberg+when:2d&hl=en-US&gl=US&ceid=US:en', label: 'Bloomberg' },
  { url: 'https://news.google.com/rss/search?q=(gold+OR+bullion)+source:reuters+when:2d&hl=en-US&gl=US&ceid=US:en', label: 'Reuters' },
  { url: 'https://news.google.com/rss/search?q=(gold+OR+%22federal+reserve%22)+source:kitco+when:2d&hl=en-US&gl=US&ceid=US:en', label: 'Kitco' },
  { url: 'https://www.theguardian.com/business/gold/rss', label: 'The Guardian', topicFilter: true },
  { url: 'https://www.mining.com/feed/', label: 'Mining.com', topicFilter: true },
];

// Per-asset news config — lanes, keyword tiers, firehose topic filter, Parallel
// objective, disk-cache key, and the LLM prompt variant (see lib/llm.js).
const ASSETS = {
  oil: {
    feeds: OIL_FEEDS,
    tier1: OIL_TIER1,
    ...
    parallel: {
      ...
      queries: ['crude oil price news today', 'OPEC Brent supply disruption latest'],
    },
  },
  gold: {
    feeds: GOLD_FEEDS,
    ...
    parallel: {
      objective:
        'Breaking news from the past 24-48 hours moving the gold price: Fed policy and rate expectations, inflation data, dollar moves, central-bank gold buying, ETF flows, geopolitical safe-haven demand. Only recent news reports — no explainers or evergreen pages.',
      queries: ['gold price news today', 'Fed rates gold bullion latest'],
    },
  },
};

async function fetchRss(feed, topic) {
  const res = await fetchWithRetry(feed.url, {}, 2);
  let items = parseRss(await res.text(), feed.label);
  if (feed.topicFilter) items = items.filter((it) => topic.test(it.title));
  return items;
}

async function fetchParallel(apiKey, cfg) {
...
async function fetchRawNews(assetKey) {
  const cfg = ASSETS[assetKey];
  const cached = readCache(cfg.cacheKey, NEWS_TTL_MS);
  if (cached) return cached;

  const apiKey = process.env.PARALLEL_API_KEY || '';
  const jobs = cfg.feeds.map((f) => fetchRss(f, cfg.topic));
  if (apiKey) jobs.push(fetchParallel(apiKey, cfg.parallel));

  const settled = await Promise.allSettled(jobs);
  const lanes = {
    rss: settled.slice(0, cfg.feeds.length).some((s) => s.status === 'fulfilled'),
  ...
  const scored = merged.map((it) => {
    const { pts, tags } = scoreText(it.title + ' ' + (it.excerpt || ''), cfg.tier1, cfg.tier2);
...
// assetKey: 'oil' (default, back-compat) or 'gold'.
async function fetchNews(model, assetKey = 'oil') {
  if (!ASSETS[assetKey]) throw new Error(`unknown news asset: ${assetKey}`);
  const raw = await fetchRawNews(assetKey);
  const now = Date.now();

  let llm = { ok: false, reason: 'disabled' };
  try {
    llm = await scoreNews(raw.items, model || DEFAULT_MODEL, process.env.OPENROUTER_API_KEY || '', ASSETS[assetKey].llmAsset);
  } catch (e) {

...
function computeSignal({ bundles, vols, news, livePrice, prevPriceHourAgo, journalStats, curve }) {
  ...
  // Curve is physical truth — never damped by the news tape (PRD keeps curve
  // signals active during events; it is what confirms or denies the headlines).
  const sCurve = curve && isFinite(curve.score) ? curve.score : null;
  ...
  const components = [
    ...
    { key: 'curve', label: 'Curve M1−M2 (physical)', s: sCurve, w: 0.15, gate: 1 },
  ].filter((c) => c.s != null);

...
- **Bug (`lib/bot.js` ~L164, `tick`)**: the daily-loss worst-case gate references `size` before its `const size = …` declaration (~L176), a temporal-dead-zone `ReferenceError`. It throws on every auto-entry that clears the earlier gates, so the caught error (`bot tick failed`) means the **auto-bot never actually opens a position**; only `manual()` (which computes `mSize` first) can enter. Move the `size`/`entryRef` computation above the gate to fix.
